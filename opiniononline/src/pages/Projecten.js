    ...
    useEffect(() => {
        async function getSurveys() {
            try {
                ...
                const { data, error } = await supabase.from('Surveys2').select('*').eq('ownerId', loggedInUserId).order('created_at')
    ...
    return (
        <div className="w-full p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 p-5 ">
                ...
                {surveys.map((survey) => {
                    return <ProjectCard key={survey.id} survey={survey} isActive={activeStatusPicker === survey.id} onActivePicker={() => setActiveStatusPicker(prev => prev === survey.id ? null : survey.id)} />
                })}
