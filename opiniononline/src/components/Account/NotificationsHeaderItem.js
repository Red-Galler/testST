...
function NotificationsHeaderItem({ notification, onDelete }) {

    // Function to format date to "time ago" format in Dutch
    const getTimeAgo = (date) => {
        // Assuming `date` is a Date object or a timestamp string that date-fns can parse
    ...
    async function FetchPicture(filename) {
        try {
            ...
            let publicUrl = `${data.publicUrl}?cb=${new Date().getTime()}`
    ...
    useEffect(() => {
        ...
        let filename = `profile_${notification.surveyTaker}`

        FetchPicture(filename);
    ...
    return (
        <MenuItem
            ...
            className={`flex items-center gap-4 py-2 pl-2 pr-8`}>
                <img className=" w-16 h-16" src={profilePicture ? profilePicture : '/default-avatar.png'} alt="avatar"></img>
            <div className="flex flex-col gap-1">
                <Typography variant="small" color="gray" className="font-semibold">
                    {notification.Users2.email} heeft {notification.Surveys2.title} uitgevoerd
                </Typography>
                <div className="flex justify-between items-center">
                    ...
                    <FaTimes onClick={onDelete} className="text-2xl text-gray-700" />
